#!/usr/bin/env python3
"""
Fast Neptune bulk-load CSV generator with optimized filtering.
Uses spatial and temporal indexing to avoid full cartesian product.
"""
...
# Build index: group UIDs by state and sex for faster lookup
...
for idx, r in up.iterrows():
    ...
    key = (state, sex)
    uid_index[key].append(idx)
    # Also add wildcard for unknown sex
    if sex:
        uid_index[(state, '')].append(idx)

print(f"Index created with {len(uid_index)} buckets")
...
# Generate edges with optimized lookup
...
with open(edges_path, 'w', newline='', encoding='utf-8') as f:
    ...
    for i, mp_r in mp.iterrows():
        ...
        # Get candidate UIDs from index (pre-filtered by state/sex)
        mp_state = str(mp_r.get('state', '')).upper().strip()
        mp_sex = str(mp_r.get('sex', '')).upper().strip()[:1] if pd.notna(mp_r.get('sex')) else ''

        # Try exact match first, then wildcard
        candidate_indices = uid_index.get((mp_state, mp_sex), [])
        if not candidate_indices and mp_sex:
            candidate_indices = uid_index.get((mp_state, ''), [])

        if not candidate_indices:
            continue

        for uid_idx in candidate_indices:
            ...
            if pd.notna(mp_r.get('date_missing')) and pd.notna(up_r.get('date_found')):
                ...
                if days_gap < -7:  # Found before missing (with tolerance)
                    continue
                if days_gap > DATE_WINDOW_DAYS:  # Too far in future
                    continue
